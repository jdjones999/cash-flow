...
export function App() {
  ...
  const renderCalendar = () => {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', marginTop: '1rem' }}>
        <div style={{ backgroundColor: '#131b2e', borderRadius: '12px', border: '1px solid #1e293b', padding: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '0.5rem', textAlign: 'center' }}>
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
              <div key={day} style={{ fontSize: '0.75rem', fontWeight: '700', color: '#64748b', paddingBottom: '0.5rem' }}>{day}</div>
            ))}

            {days.map(day => {
              const dayEvents = items.filter(i => i.dueDate === day);
              ...
              return (
                ...
                >
                  <span style={{ fontSize: '0.75rem', fontWeight: '600', color: isSelected ? '#38bdf8' : '#94a3b8' }}>{day}</span>
                  {dayEvents.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2px' }}>
                      {dayEvents.map(evt => (
                        <div key={evt.id} style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: evt.category === 'income' ? '#4ade80' : '#f87171' }} />
                      ))}
                    </div>
                  )}
                </div>
        ...
        <div style={{ backgroundColor: '#131b2e', borderRadius: '12px', border: '1px solid #1e293b', padding: '1.5rem' }}>
          <h4 style={{ fontSize: '1rem', fontWeight: '600', color: '#f8fafc', marginTop: 0, marginBottom: '1rem' }}>
            Details for Day {selectedDay}
          </h4>

          {selectedDayItems.length === 0 ? (
            <p style={{ color: '#64748b', fontSize: '0.85rem' }}>No scheduled cash flow events for this date.</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {selectedDayItems.map(item => (
                <div key={item.id} style={{ backgroundColor: '#0b0f19', border: '1px solid #1e293b', padding: '0.75rem', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: '0.85rem', fontWeight: '600', color: '#f8fafc' }}>{item.name}</div>
                    <span style={{ fontSize: '0.7rem', color: '#64748b' }}>{item.category.replace('_', ' ')}</span>
                  </div>
                  <span style={{ fontWeight: '700', fontSize: '0.875rem', color: item.category === 'income' ? '#4ade80' : '#f87171' }}>
                    ${item.amount.toFixed(2)}
                  </span>
                </div>
              ))}
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0b0f19', color: '#f8fafc', fontFamily: 'Inter, system-ui, sans-serif' }}>
      
      {/* Header */}
      <header style={{ borderBottom: '1px solid #1e293b', padding: '1rem 2rem', backgroundColor: '#0f172a', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <h1 style={{ fontSize: '1.25rem', fontWeight: '700', margin: 0 }}>Cash Flow</h1>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
            Signed in as <strong style={{ color: '#f8fafc' }}>{currentUser.email}</strong> ({currentUser.role})
          </div>
          <button
            onClick={() => setCurrentUser(null)}
            style={{ backgroundColor: '#1e293b', border: '1px solid #334155', color: '#f8fafc', borderRadius: '6px', padding: '0.4rem 0.8rem', fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
          >
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        {/* Navigation Tabs */}
        <nav style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', borderBottom: '1px solid #1e293b', paddingBottom: '0.75rem' }}>
          <button
          ...
          >
            <DollarSign style={{ width: '16px', height: '16px' }} /> Income
          </button>
          ...
          >
            <Wallet style={{ width: '16px', height: '16px' }} /> Subscriptions
          </button>
          ...
          >
            <AlertTriangle style={{ width: '16px', height: '16px' }} /> Unexpected
          </button>
          ...
          >
            <CalendarIcon style={{ width: '16px', height: '16px' }} /> Calendar
          </button>
          ...
          {currentUser.role === 'admin' && (
            ...
            >
              <Settings style={{ width: '16px', height: '16px' }} /> Admin Settings
            </button>
          )}
        </nav>
        ...
        {activeTab === 'income' && renderItemTable(items.filter(i => i.category === 'income'), 'Income', 'income', 'monthly')}
        {activeTab === 'subscriptions' && renderItemTable(items.filter(i => i.category === 'subscription'), 'Subscriptions', 'subscription', 'monthly')}
        {activeTab === 'unexpected' && renderItemTable(items.filter(i => i.category === 'unexpected_expense'), 'Unexpected Expenses', 'unexpected_expense', 'one_time')}
